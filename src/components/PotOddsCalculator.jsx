...
export default function PotOddsCalculator({ isLoading, potSize, setPotSize, callAmount, setCallAmount, equityHistory, windowSize, isPc = false }) {
  ...
  const dynamicPadding = isPc ? "5px 8px" : `${Math.max(4, Math.round(vh * 0.8))}px ${Math.max(6, Math.round(vh * 1.0))}px`;
  const dynamicInnerPadding = isPc ? "3px 6px" : `${Math.max(4, Math.round(vh * 0.8))}px ${Math.max(6, Math.round(vh * 1.0))}px`;
  const dynamicMarginTop = isPc ? "2px" : `${Math.max(4, Math.round(vh * 0.8))}px`;
  const dynamicGap = isPc ? "4px" : `${Math.max(4, Math.round(vh * 0.6))}px`;
  const dynamicInputPadding = isPc ? "2px 6px" : `${Math.max(2, Math.round(vh * 0.5))}px ${Math.max(4, Math.round(vh * 0.8))}px`;
  const dynamicTitleFontSize = isPc ? "12px" : "clamp(10px, 12vh, 13px)";
  const dynamicLabelFontSize = isPc ? "10.5px" : "clamp(8.5px, 10vh, 11px)";
  const dynamicValueFontSize = isPc ? "13px" : "clamp(11px, 14vh, 15px)";
  const dynamicButtonFontSize = isPc ? "10px" : "clamp(8.5px, 10vh, 11px)";
    ...
    <div style={{ marginTop: dynamicMarginTop, backgroundColor: "white", padding: dynamicPadding, borderRadius: "8px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid #e2e8f0", textAlign: "left", boxSizing: "border-box" }}>
      ...
      <div
        ...
        style={{
          ...
          paddingBottom: isOpen ? "0.4vh" : "0",
          marginBottom: isOpen ? dynamicMarginTop : "0"
        }}
      >
        <h3 style={{ margin: 0, color: "#1e293b", fontSize: dynamicTitleFontSize, fontWeight: "bold" }}>必要勝率計算機</h3>
        <span style={{ fontSize: dynamicLabelFontSize, color: "#64748b", fontWeight: "bold" }}>
          {isOpen ? "▲ 折りたたむ" : "▼ 開く"}
        </span>
      </div>
